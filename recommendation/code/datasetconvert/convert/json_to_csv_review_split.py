...
def get_row(line_contents, column_names, userId_c, businessId_c, review_id):
    """Return a csv compatible row given column names and a dict."""
    row = []
    review_row = []
    review_row.append(review_id)
    for column_name in column_names:
        ...
        if column_name == 'user_id':
            ...
            row.append(line_value)
        elif column_name == 'stars':
            row.append(line_value)
        elif column_name == 'text':
            if isinstance(line_value, unicode):
                review_row.append('{0}'.format(line_value.encode('utf-8')))
            elif line_value is not None:
                review_row.append('{0}'.format(line_value))
            else:
                review_row.append('')
    row.append(review_id)
    #print 'column_names: ' + str(column_names)
    #print 'row: ' + str(row)
    return row, review_row
...
def write_file(file_path, review_file_path, file_contents, column_names, userId_c, businessId_c):
    """Create and write a csv file given file_contents of our json dataset file and column names."""
    csv_file = csv.writer(open('file_path', 'wb+'))
    review_csv_file = csv.writer(open('review_file_path', 'wb+'))
    with open(file_path, 'wb+') as fin:
        csv_file = csv.writer(fin)
        with open(review_file_path, 'wb+') as fin1:
            review_csv_file = csv.writer(fin1)
            csv_file.writerow(column_names[0:len(column_names)-1] + ['review_id'])
            review_csv_file.writerow(['review_id'] + column_names[len(column_names)-1:])
            review_id = 1
            #csv_file.writerow(list(column_names))
            for line_contents in file_contents:
                row, review_row = get_row(line_contents, column_names, userId_c, businessId_c, review_id)
                review_id += 1
                #print 'row: ' + str(row)
                #print 'review_row: ' + str(review_row)
                csv_file.writerow(row)
                review_csv_file.writerow(review_row)
        #csv_file.writerow(get_row(line_contents, column_names[0:len(column_names)-1], userId_c, businessId_c))
...
if __name__ == '__main__':
    ...
    csv_review_file = '{0}txt.csv'.format(json_file.split('.json')[0])
    print csv_file
    print csv_review_file
    file_contents, column_names = read_file(json_file)
    #print column_names
    column_names = ['user_id', 'business_id', 'stars', 'date', 'text']
    #column_names = ['user_id', 'business_id', 'stars', 'date']
    write_file(csv_file, csv_review_file, file_contents, column_names, userId_c, businessId_c)
